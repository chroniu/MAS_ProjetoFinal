...
#    plot.x_range = (min(df['date']), max(df['date']))
#    plot.y_range = (min(df['date']), max(df['date'])) 
...
# define os gráficos
plot = figure(plot_height=110, tools="", toolbar_location=None, #name="line",
           x_axis_type="datetime", sizing_mode="scale_width")
plot.line('date', 'us_max', source=source, line_width=2, alpha=0.7)

"""select = figure(plot_height=50, plot_width=800, y_range=plot.y_range,
...
select.line('date', 'us_max', source=source)
...
# define a tabela
columns = [
    TableColumn(field="date", title="Data", formatter=DateFormatter()),
